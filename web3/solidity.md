...
## 基本数据类型有哪些
1. 值类型：变量直接存储具体数值，赋值时拷贝完整数据，无需显式指定存储位置（memory/storage）
  + 布尔型（bool）
  + 整型（int、uint）：
  ...
  + 字节型（byte）
  + 枚举型（enum）
2. 引用类型：变量存储数据的引用（地址），赋值时仅拷贝引用，必须显式指定存储位置（memory/storage/calldata），否则编译报错
  + 字符串型（string）
  + 数组型（array）
  + 结构体型（struct）
  + 映射型（mapping）
...
### 全局变量：msg
1. `msg.sender`: 调用合约的地址
2. `msg.value`: 调用合约的交易金额（以太币）
  + 当合约被外部账户调用时，`msg.value` 就是调用合约的交易金额。
  + 当合约被交易调用时，`msg.value` 就是发送交易的交易金额。
  + 当合约A被合约B调用时，`msg.sender` 就是合约B的地址。
### 全局变量：block
1. `block` 是一个全局变量，用于获取当前区块的信息：区块高度、时间戳、难度、挖矿奖励等。
  + `block.number` 可以获取当前区块的高度。
  + `block.timestamp` 可以获取当前区块的时间戳。
  + `block.difficulty` 可以获取当前区块的难度。
  + `block.coinbase` 可以获取当前区块的挖矿奖励地址。
### 全局变量：tx
1. `tx.origin`：交易的发起地址
2. `tx.gasprice`：交易的 Gas 价格
3. `tx.value`：交易的价值（以太币）

## 变量修饰符：storage、memory、calldata 的区别？
- storage：
  - 存储在合约的状态变量中，永久存在于合约的存储中。
  - 可读写。gas成本高
  - 引用传递、浅拷贝
- memory：
  - 存储在函数调用期间的临时内存中，函数调用结束后会被清除。
  - 可读写。gas成本较低
  - 值传递、深拷贝
- calldata：存储在函数调用期间的临时内存中，函数调用结束后会被清除。只读。gas成本低（只能用于参数，不能用于返回值）

## 函数的可见性修饰符有哪些？
- public：公开的，合约内部和外部都能调用。
...
## require和revert+error的区别
1. 相同点：都是用于触发交易回滚的核心机制
2. 不同点：
  + revert + error 更省燃气
