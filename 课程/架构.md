...
3. 框架排名：https://risingstars.js.org/2024/en

## 前端框架
1. htmx
    + 通过在 HTML 中添加属性，直接发起 AJAX 请求，更新部分页面内容，实现动态交互
    + 优点：简单、轻量、易于集成到现有项目中
    + 适合“以展示为主、交互简单”的页面
2. react
    + 组件化、声明式 UI 构建
...
1. mui
    ...
    + 快速搭建应用，不适合深度定制
2. shadcn/ui
    + 给你“源码级控制权”的组件集合，安装组件的时候直接把源码拉到你项目里
...
3. twailwind css
    ...
    + 通过原子化类名快速构建自定义设计
4. react bits
    + 一个开源的、高质量的、带动画交互效果的 React 组件合集，帮助你构建惊艳的用户界面
...
## Bun
1. 集成了运行时 + 打包器 + 包管理器的 JS 运行环境，你可以把它理解成：Node.js + npm + esbuild + jest = Bun
2. Bun 的特点：
    + 高性能：bun 的运行速度通常比 Node.js 和 Deno 更快，尤其在启动时间和包管理方面表现突出。
    + 一体化工具链：bun 集成了打包器、测试框架和包管理器，减少了对外部工具的依赖。
    + 兼容性：bun 兼容大部分 Node.js API，使得现有的 Node.js 项目可以较为容易地迁移到 bun。
