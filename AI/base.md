...
## agent
1. 能自己“规划 + 决策 + 调用工具 + 执行”的 AI 工作者
## mcp
1. 给 AI 接“外部系统/工具/数据源”的标准协议
## skill
1. 可复用的能力模块/工具包，通常是把某个任务流程标准化，让 AI 或系统可以稳定调用。把某类任务做成可复用流程，方便其他项目调用。
   + 输入规范（参数、约束、默认值）
   + 执行逻辑（提示词流程 / 程序逻辑 / 工具调用）
   + 输出规范（结构化结果、错误码、产物链接等）
## 关系
Agent（大脑） 通过 MCP（接口） 调用各种 Skill/工具（手和脚）
