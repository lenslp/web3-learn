## 多个模块如何整合到一起的
1. 加载子应用（HTML Entry 解析器 + fetch）
  + 在主应用中注册子应用（name、entry、activeRule、container）
  + qiankun监听路由变化，匹配到路由（activeRule）之后，通过fetch(entry)获取子应用html
  + 解析html，动态加载html中引入的js和css
```typescript
// 动态加载 CSS
const loadCSS = (href) => {
...
loadJS('/path/to/app.js');
```
2. 创建子应用运行沙箱
  + js隔离：
    - 快照沙箱：子应用激活时，深拷贝window，创建快照对象
    - proxy沙箱：proxy代理window对象
  + css隔离：
    - 创建shadow dom
    - css modules
    - 子应用样式动态添加前缀
3. 将生命周期（bootstrap/mount/unmount）组装到主应用中
  + qiankun给每个子应用都定义了一套生命周期钩子，确保在应用加载、激活、卸载的过程中能正确管理和隔离资源。
  + 每个子应用都需要实现三个核心生命周期函数（umi框架默认集成）
    - bootstrap：初始化
    - mount：挂载，将子应用渲染到页面的指定容器中。
    - unmount：卸载子应用，移除dom
```typescript
// 挂载
