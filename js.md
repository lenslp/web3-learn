...
## 事件循环
1. JavaScript 是单线程的，为了避免阻塞主线程，引入了事件循环机制。
2. 执行主线程中的同步代码。
3. 遇到微任务（如 Promise.then、async/await）放入当前的微任务队列，遇到宏任务（如 setTimeout、UI 事件、I/O 等）则放入宏任务队列。
4. 清空当前微任务队列，执行所有微任务，直到微任务队列为空。
5. 从宏任务队列取出第一个宏任务并执行（例如 setTimeout、setInterval 等）。
6. 清空当前的微任务队列。
7. 微任务执行完毕后会检查是否需要渲染，若到达渲染时机则执行，否则直接进入下一轮循环。
