...
2. 架构设计：负载检测层、路由决策层、渲染执行层
  ...
  + 路由决策层：利用 Next.js 中间件（Middleware）在请求入口处拦截，根据负载和页面优先级决定渲染方式，核心页面（如行情页、交易页）优先 SSR 渲染，其他页面根据负载情况动态切换为 CSR 渲染。
    - 方案 A：重定向到预渲染的静态降级页（适合内容变化少的页面）：response.redirect('/static-degrade-page')
    - 方案 B：强制使用客户端渲染（通过 cookie 告知页面）：response.cookies.set('force-csr', 'true', { maxAge: 5 })
...
通过以上方案，可实现交易所项目的 “高性能渲染 + 强容灾能力 + 高效团队协作”，核心是围绕 “交易可用性” 设计降级策略，围绕 “用户体验” 优化渲染链路，围绕 “开发效率” 通过微前端和 BFF 解耦业务。